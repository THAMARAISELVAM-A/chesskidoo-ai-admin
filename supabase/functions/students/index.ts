  ...
  try {
    ...
    if (method === 'PUT') {
      ...
      if (rawBody.payment_status !== undefined) {
        // Only update the payment_status column. Do NOT derive or overwrite the
        // enrollment `status`/`account_status` from the payment value — these are
        // independent fields. Previously, marking a payment as "Pending" silently
        // flipped the enrollment status to 'pending' (shown as "Not Enrolled"/"Pending").
        updateData.payment_status = String(rawBody.payment_status);
      }
